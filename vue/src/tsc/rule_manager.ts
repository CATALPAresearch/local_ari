    ...
    for (let i = 0; i < augmentations.length; i++) {
      switch (augmentations[i]) {
        ...
        case EActionAugmentation.RELATED_RESOURCE:
          ...
          let request_array: object[] = [];
          let req = {};
          // iterate over the allowed resource keys and create a request array
          for (
            let key = 0;
            ...
            if (text.match(re)) {
              switch (allowed_rel_resource_keys[key]) {
                case "rec.safran-like-completed-assignments":
                  req = {
                    input_component: "mod_assign",
                    input_condition: "completed", // new | completed | ...
                    input_instance_id: 0,
                    input_component_section: 0,
                    output_component: "mod_safran",
                    number_of_results: 3,
                  };
                  break;
              ...
              request_array[String(text.match(re))] = req;
            }
          }
