...
export class StoredPrompt {
    ...
    constructor(config:IStoredPromptConfig){
        ...
        openRequest.onsuccess = function(){
            ...
            console.log("ARI::Prompt added to the indexed db store", request.result);
            };
            
            request.onerror = function() {
            console.log("ARI::PromptError", request.error);
            };
    ...
    private _guard(config:IStoredPromptConfig):boolean{
        ...
        if(typeof config.message !== "undefined" && (typeof config.message !== "string" || config.message.length <= 0)) return false;
        if(typeof config.title !== "undefined" && (typeof config.title !== "string" || config.title.length <= 0)) return false;
        return true;
    }
}
