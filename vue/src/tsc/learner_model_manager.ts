...
export class LearnerModelManager {
  private static wwwroot: string;
  private user_id: number;
  private static course_id: number;
  ...
  constructor(wwwroot: string, user_id: number, course_id: number) {
    LearnerModelManager.wwwroot = wwwroot;
    LearnerModelManager.course_id = course_id;
  ...
  public checkRules(): void {
    new RuleManager(
      ...
      LearnerModelManager.course_id,
      LearnerModelManager.wwwroot
    );
  }
  ...
  public getLearnerModel(): void {
    ...
    var path: string =
    LearnerModelManager.wwwroot +
      "/local/ari/lm/learner_model.php?format=json&&period=WS23&course_id=" +
      LearnerModelManager.course_id +
      "&user_id=" +
      this.user_id;
    //console.log(path);
...
export interface ILearnerModel {
  ...
  longpage_activity?: {
    // all measures as key-value list of longpage instance
